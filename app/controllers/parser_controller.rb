class ParserController < ApplicationController
  def wre_ids
    ...
    # wre_results
    redirect_to(runners_path(wre: true))
  end
  ...
  #   index = competition_data.find_index { |td| td.text[/PROCES VERBAL/i]} || -1
  ...
  #     if clasa_tr = group.detect { |tr| tr.text[/Clasa( |)distanț/]}
  #       tds = clasa_tr.css("td:not(:empty)")
  #       tds_index = tds.find_index { |td| td.text[/Clasa( |)distanț/]}

  #       group_data[:clasa] = get_clasa(tds[tds_index + 1].text)

  #     else
  #       group_data[:clasa] ="Fara Categorii"
  #     end
  ...
  #         if result[headers_index[:dob]][/\d{2}\.\d{2}\.\d{2}/]
  #            Date.strptime(result[headers_index[:dob]], "%d.%m.%y")
  #          else
  ...
  #       end.to_date || Time.now.to_date).as_json rescue byebug
  #       next if place < 1
  ...
  #         dob: dob,
  #         # club: re sult[headers_index[:club]],
  ...
  #     group = Group.find_or_create_by(competition: competition, group_name: group_data[:group_name]) do |group|
  #       group.competition = competition
  #       group.group_name = group_data[:group_name]
  #       group.clasa = group_data[:clasa]
  #     end
  ...
  #       runner_id = Runner.get_runner(runner).id
